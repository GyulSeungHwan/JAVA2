...
public class ForExample01 {
	public static void main(String[] args) {
		...
		//입력한 값에 대한 총합, 평균, 최대값, 데이터 받을 개수(반복문 갯수) 스캐너 활용해서 데이터 입력
		//총합 : 입력 받자마자 총합이라는 변수에 누적 합계
		//평균 : 총합 / 데이터 입력 갯수
		//최대값 : 입력한 데이터와 기존에 들어있는 데이터 비교 더 큰거
		//최소값 : 입력한 데이터와 기존에 들어있는 데이터 비교 더 작은거
...
//			total = total + data;
		...
		for(int i=0; i<5; i++) {
			...
			if(randomNo == num) {
				System.out.println(i + "번째의 기회에 맞추셨습니다.");
				break;
			...
			if(i == 5) {
				System.out.println("5번의 기회를 모두 소진하였습니다.");
			}			
		}
	}
}
